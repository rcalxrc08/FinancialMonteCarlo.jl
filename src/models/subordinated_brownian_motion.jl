...
function simulate(mcProcess::SubordinatedBrownianMotion,spotData::equitySpotData,mcBaseData::MonteCarloConfiguration{type1,type2,type3},T::numb,dt_s::Array{num1,2},monteCarloMode::MonteCarloMode=standard,parallelMode::SerialMode=SerialMode()) where {numb <: Number, num1<:Number , type1 <: Number, type2<: Number, type3 <: StandardMC}
...
function simulate(mcProcess::SubordinatedBrownianMotion,spotData::equitySpotData,mcBaseData::MonteCarloConfiguration{type1,type2,type3},T::numb,dt_s::Array{num1,2},monteCarloMode::MonteCarloMode=standard,parallelMode::SerialMode=SerialMode()) where {numb <: Number, num1<:Number , type1 <: Number, type2<: Number, type3 <: AntitheticMC}
	Nsim=mcBaseData.Nsim;
	Nstep=mcBaseData.Nstep;
	if(size(dt_s)!=(Nsim,Nstep))
		error("Inconsistent Time Matrix")
