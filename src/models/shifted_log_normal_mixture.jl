...
function simulate(mcProcess::ShiftedLogNormalMixture,spotData::equitySpotData,mcBaseData::MonteCarloConfiguration{type1,type2,type3},T::numb,parallelMode::BaseMode=SerialMode()) where {numb <: Number, type1 <: Number, type2<: Number, type3 <: AbstractMonteCarloMethod}
	if T<=0.0
	...
	dt=T/Nstep
	A0=S0*(1-mcProcess.α)
	S=(λ_gmb[1]*A0).*simulate(GeometricBrownianMotion(η_gbm[1],mu_gbm),equitySpotData(A0,r,d),mcBaseData,T,parallelMode);
	for (η_gbm_,λ_gmb_) in zip(η_gbm[2:end],λ_gmb[2:end])
		S+=(λ_gmb_*A0).*simulate(GeometricBrownianMotion(η_gbm_,mu_gbm),equitySpotData(A0,r,d),mcBaseData,T,parallelMode)
	end
