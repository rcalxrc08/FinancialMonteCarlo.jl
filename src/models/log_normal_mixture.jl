...
function simulate(mcProcess::LogNormalMixture,spotData::equitySpotData,mcBaseData::MonteCarloConfiguration{type1,type2,type3},T::numb,monteCarloMode::MonteCarloMode=standard,parallelMode::BaseMode=SerialMode()) where {numb <: Number, type1 <: Number, type2<: Number, type3 <: AbstractMonteCarloMethod}
	if T<=0.0
	...
	mu_gbm=r-d;
	S=λ_gmb[1]*S0.*simulate(GeometricBrownianMotion(η_gbm[1],mu_gbm),spotData,mcBaseData,T,monteCarloMode,parallelMode);
	for (η_gbm_,λ_gmb_) in zip(η_gbm[2:end],λ_gmb[2:end])
		S+=λ_gmb_*S0.*simulate(GeometricBrownianMotion(η_gbm_,mu_gbm),spotData,mcBaseData,T,monteCarloMode,parallelMode)
	end
	return S;
