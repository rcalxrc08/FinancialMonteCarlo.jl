...
f(x::Vector) = pricer(BlackScholesProcess(x[1]),ZeroRate(x[2],x[3],x[4]),mc,EuropeanOption(x[5],K),FinancialMonteCarlo.CudaMode_2());
...
f_(x::Vector) = pricer(BlackScholesProcess(x[1]),ZeroRate(x[2],x[3],x[4]),mc,AmericanOption(x[5],K),FinancialMonteCarlo.CudaMode_2());
