...
f(x) = pricer(BlackScholesProcess(x[1]),ZeroRateCurve(x[2],r,d),mc,EuropeanOption(x[3],K),Underlying(S0,d));
...
f1_(x) = pricer(BlackScholesProcess(x[1]),ZeroRateCurve(x[2],r,d),mc,AmericanOption(T,K),Underlying(S0,d));
